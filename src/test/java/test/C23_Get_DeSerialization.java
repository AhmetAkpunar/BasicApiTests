package test;

import baseUrl.DummyBaseUrl;
import io.restassured.response.Response;
import org.junit.Assert;
import org.junit.Test;
import testData.TestDataDummy;

import java.util.Map;

import static io.restassured.RestAssured.given;

public class C23_Get_DeSerialization extends DummyBaseUrl {

    /*
    http://dummy.restapiexample.com/api/v1/employee/3 url’ine bir GET
    request gonderdigimizde donen response’un status code’unun 200,
    content Type’inin application/json ve body’sinin asagidaki gibi
    oldugunu test edin.
    Response Body
    {
    "status":"success",
    "data":{
            "id":3,
            "employee_name":"Ashton Cox",
            "employee_salary":86000,
            "employee_age":66,
            "profile_image":""
            },
    "message":"Successfully! Record has been fetched."
    }

     */

    @Test
    public void get01(){

        // 1- Url hazırla
        specDummy.pathParams("pp1","employee","pp2",3);

        // 2- Expected Data hazırla

        TestDataDummy testDataDummy = new TestDataDummy();
        Map<String,Object> expDataMap = testDataDummy.expBodyOlusturMap();
        System.out.println("ExpDataMap : " + expDataMap);

        // 3- Response'u kaydet

        Response response = given().spec(specDummy).when().get("/{pp1}/{pp2}");

        System.out.println("====================================================");
        response.prettyPrint();

        // 4- Assertion

        Assert.assertEquals(testDataDummy.basariliStatusCode,response.getStatusCode());
        Assert.assertEquals(testDataDummy.contentType,response.contentType());

        Map<String,Object> respMap = response.as(Map.class);

        Assert.assertEquals(expDataMap.get("status"),respMap.get("status"));
        Assert.assertEquals(expDataMap.get("message"),respMap.get("message"));
        Assert.assertEquals(((Map)(expDataMap.get("data"))).get("id"),
                ((Map)(respMap.get("data"))).get("id"));
        Assert.assertEquals(((Map)(expDataMap.get("data"))).get("employee_name"),
                ((Map)(respMap.get("data"))).get("employee_name"));
        Assert.assertEquals(((Map)(expDataMap.get("data"))).get("employee_salary"),
                ((Map)(respMap.get("data"))).get("employee_salary"));
        Assert.assertEquals(((Map)(expDataMap.get("data"))).get("employee_age"),
                ((Map)(respMap.get("data"))).get("employee_age"));
        Assert.assertEquals(((Map)(expDataMap.get("data"))).get("profile_image"),
                ((Map)(respMap.get("data"))).get("profile_image"));

    }
}
